...
from torch_geometric.nn import TAGConv, global_add_pool, global_max_pool, DynamicEdgeConv
...
def get_mlp_layer(in_channels, out_channels):
...
class GCN(GNN):
    def __init__(self, nb_inputs, nb_outputs, nb_intermediate=12, dropout_ratio=0.3):
        """ConvNet model.
        Args:
        ...
        # Architecture configuration
        self.conv1 = DynamicEdgeConv(get_mlp_layer(self.nb_inputs, self.nb_intermediate), self.nearest_nb)
        self.conv2 = DynamicEdgeConv(get_mlp_layer(self.nb_intermediate, self.nb_intermediate), self.nearest_nb)
        self.conv3 = DynamicEdgeConv(get_mlp_layer(self.nb_intermediate, self.nb_intermediate), self.nearest_nb)
    ...
    #def forward(self, x, edge_index, batch):
        ...
        x = F.leaky_relu(self.conv1(x, batch))
        x1 = torch.cat([
            global_add_pool(x, batch),
            global_max_pool(x, batch),
        ], dim=1)
        x = F.leaky_relu(self.conv2(x, batch))
        x2 = torch.cat([
            global_add_pool(x, batch),
            global_max_pool(x, batch),
        ], dim=1)

        x = F.leaky_relu(self.conv3(x, batch))
        x3 = torch.cat([
            global_add_pool(x, batch),
            global_max_pool(x, batch),
        ], dim=1)
...
if __name__ == '__main__':
    ...
    analysis_type = 'mono_full_camera'
    indir = '/home/woody/caph/mppi067h/gamma_ray_reconstruction_with_ml/gnn/hess_datasets/phase2d3/'
    outdir = '/home/saturn/caph/mppi067h/graph_datasets/test/' +  analysis_type
    out_model_dir = '/home/woody/caph/mppi067h/gamma_ray_reconstruction_with_ml/gnn/trained_models/test/' + analysis_type
    ...
    start_epoch = 1
    num_epochs = 100
    batch_sizes = {'mono_full_camera': 1024,
                   'stereo': 512,
